...
#loads compiled C .so file inyo puyhon and returns an obj where every C function in library becomes an attribute
...
# GAME_ENGINE FUNCTION SIGNATURES


# Create and Destroy game engine
...
lib.game_engine_get_room_dimensions.argtypes = [
    GameEngine, ctypes.POINTER(ctypes.c_int), ctypes.POINTER(ctypes.c_int)
]
...
# not exposed but declared
lib.game_engine_reset.argtypes = [GameEngine]
lib.game_engine_reset.restype = ctypes.c_int

lib.game_engine_get_treasure_count.argtypes = [GameEngine, ctypes.POINTER(ctypes.c_int)]
lib.game_engine_get_treasure_count.restype = ctypes.c_int

lib.game_engine_get_adj_mtx.argtypes = [GameEngine, ctypes.POINTER(ctypes.POINTER(ctypes.c_int)), ctypes.POINTER(ctypes.c_int)]
lib.game_engine_get_adj_mtx.restype = ctypes.c_int
...
lib.player_set_position.restype = ctypes.c_int
...
# This is the game engine free string void type and void pointer
lib.game_engine_free_string.argtypes = [ctypes.c_void_p]
lib.game_engine_free_string.restype = None

# this is destroy_treasure void type and treasure poinrter
